...
export function Stages() {
  ...
  return (
    <section id="stages" className="bg-navy py-10 lg:py-20">
      <Container>
        <SectionHeading
          ...
          {dict.stages.items.map((item, index) => (
            <li key={item.title} className="relative">
              {index < dict.stages.items.length - 1 ? (
                <span className="absolute left-10 top-6 hidden h-px w-[calc(100%-0.5rem)] bg-gold/40 md:block" />
              ) : null}
              <div className="relative h-full border border-white/10 bg-navy-dark p-6">
                <p className="font-serif text-4xl font-bold leading-none text-[#C9A24B]">
                  {index + 1}
                </p>
